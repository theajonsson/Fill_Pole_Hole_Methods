...
"""
def format_SIT_outside(file_path, lat_level=60, max_lat_level=81.5):
...
def polehole(year, month, debug=False):
    ...
    for files_SIC in files_SIC:
        x_SIC, y_SIC, SIC = fd.format_SIC(os.path.join(folder_path_SIC,files_SIC))
    ...
    V_cell_in = ((SIT_pred*1e-3)*(SIC_mean/100))*area_cell
    ...
    mask, SIT = format_SIT_outside(file_sit)
    print(f"Mean value of outside SIT: {np.nanmean(SIT)} m")

    folder_path_SIC = str(Path(__file__).resolve().parent.parent/f"Data/SIC/{year}/{month}/")
    files_SIC = sorted([f for f in os.listdir(folder_path_SIC) if f.startswith("ice_conc_nh") and f[0].isalnum()])

    SIC_all = pd.DataFrame()
    day = 1
    for files_SIC in files_SIC:
        dataset = nc.Dataset(os.path.join(folder_path_SIC,files_SIC), "r", format="NETCDF4")
        ...
        x_SIC,y_SIC = lonlat_to_xy(lat_SIC, lon_SIC, "n")

        SIC_all[f"Day_{day}"] = SIC.flatten()
        day += 1
    SIC_mean = np.array(SIC_all.mean(axis=1))
    mask_nan = ~np.isnan(SIC_mean)
    SIC_mean = SIC_mean[mask_nan]
    SIT = SIT[mask_nan]

    V_cell_out = ((SIT/1000)*(SIC_mean/100))*area_cell
    ...
    print(f"Total volume of Arctic is: {V_total} km^3 \n")



    # Plot predicted SIV on polar map
    if debug:
        x_SIC = x_SIC[mask_nan]
        y_SIC = y_SIC[mask_nan]
